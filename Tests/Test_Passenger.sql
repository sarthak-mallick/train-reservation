...
BEGIN
    ...
    -- 1.2 NULL Middle Name (optional field)
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => 'Jane',
        p_middle_name   => NULL,
        p_last_name     => 'Smith',
    ...
    print_result('1.3 Missing First Name', v_passenger_id = -1, v_error_msg);

    -- 1.4 Whitespace-Only First Name
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => '   ',
        p_middle_name   => 'Test',
        p_last_name     => 'User',
    ...
    print_result('1.4 Whitespace-Only First Name', v_passenger_id = -1, v_error_msg);

    -- 1.5 Missing Last Name
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => 'Test',
        p_middle_name   => NULL,
        p_last_name     => NULL,
    ...
    print_result('1.5 Missing Last Name', v_passenger_id = -1, v_error_msg);

    -- 1.6 Invalid Email - No @ symbol
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => 'Test',
        p_middle_name   => NULL,
        p_last_name     => 'User',
    ...
    print_result('1.6 Invalid Email Format', v_passenger_id = -1, v_error_msg);

    -- 1.7 Invalid Phone - Too Short
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => 'Test',
        p_middle_name   => NULL,
        p_last_name     => 'User',
        ...
        p_address_state => 'MA',
        p_address_zip   => '02101',
        p_email         => 'test7@example.com',
        p_phone         => '123',
        p_error_msg     => v_error_msg
    );
    print_result('1.7 Invalid Phone (< 10 digits)', v_passenger_id = -1, v_error_msg);
    ...
    print_result('1.8 Future Date of Birth', v_passenger_id = -1, v_error_msg);
    ...
    print_result('1.9 Duplicate Email', v_passenger_id = -1, v_error_msg);

    -- 1.10 Duplicate Email (Case-Insensitive)
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => 'Duplicate',
        p_middle_name   => NULL,
        p_last_name     => 'Email',
    ...
    print_result('1.10 Duplicate Email (Case-Insensitive)', v_passenger_id = -1, v_error_msg);

    -- 1.11 Duplicate Phone
    v_passenger_id := crs_admin.PKG_PASSENGER.register_passenger(
        p_first_name    => 'Duplicate',
        p_middle_name   => NULL,
        p_last_name     => 'Phone',
    ...
    print_result('1.11 Duplicate Phone', v_passenger_id = -1, v_error_msg);
    ...
    -- 2.4 Search - Non-existent Email
    v_search_id := crs_admin.PKG_PASSENGER.search_passenger('EMAIL', 'nonexistent@example.com');
    print_result('2.4 Non-existent Email', v_search_id IS NULL, 'Returned NULL');

    -- 2.5 Search - Invalid Search Type
    v_search_id := crs_admin.PKG_PASSENGER.search_passenger('INVALID', 'test@example.com');
    print_result('2.5 Invalid Search Type', v_search_id IS NULL, 'Returned NULL');

    -- 2.6 Search - NULL Search Value
    v_search_id := crs_admin.PKG_PASSENGER.search_passenger('EMAIL', NULL);
    print_result('2.6 NULL Search Value', v_search_id IS NULL, 'Returned NULL');

    -- 2.7 Search by ID - Non-numeric
    v_search_id := crs_admin.PKG_PASSENGER.search_passenger('ID', 'abc');
    print_result('2.7 Non-numeric ID', v_search_id IS NULL, 'Returned NULL');
    ...
    -- 3.5 Email Belongs to Another Passenger
    v_search_id := crs_admin.PKG_PASSENGER.search_passenger('EMAIL', 'john.updated@example.com');
    IF v_search_id IS NOT NULL THEN
        v_result := crs_admin.PKG_PASSENGER.update_passenger(
            p_passenger_id  => v_search_id,
        ...
        print_result('3.5 Email Belongs to Another', v_result = FALSE, v_error_msg);
    END IF;

    -- 3.6 Keep Same Email (Self-Update)
    v_search_id := crs_admin.PKG_PASSENGER.search_passenger('EMAIL', 'john.updated@example.com');
    IF v_search_id IS NOT NULL THEN
        v_result := crs_admin.PKG_PASSENGER.update_passenger(
            p_passenger_id  => v_search_id,
        ...
        print_result('3.6 Keep Same Email (Self)', v_result = TRUE, 'Allowed');
    END IF;

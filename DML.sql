...
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Monday', 'N');
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Tuesday', 'N');
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Wednesday', 'N');
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Thursday', 'N');
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Friday', 'N');
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Saturday', 'Y');
INSERT INTO CRS_DAY_SCHEDULE (day_of_week, is_week_end) VALUES ('Sunday', 'Y');
COMMIT;

-- Populate CRS_TRAIN_INFO (10 sample trains)

INSERT INTO CRS_TRAIN_INFO (train_number, source_station, dest_station, total_fc_seats, total_econ_seats, fc_seat_fare, econ_seat_fare)
...
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 1, 'Y' FROM CRS_DAY_SCHEDULE;

-- TR002: Weekdays only (Monday-Friday)
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 2, 'Y' FROM CRS_DAY_SCHEDULE WHERE is_week_end = 'N';

-- TR003: All days
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 3, 'Y' FROM CRS_DAY_SCHEDULE;

-- TR004: Weekdays only
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 4, 'Y' FROM CRS_DAY_SCHEDULE WHERE is_week_end = 'N';

-- TR005: All days
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 5, 'Y' FROM CRS_DAY_SCHEDULE;

-- TR006: Weekends only (Saturday-Sunday)
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 6, 'Y' FROM CRS_DAY_SCHEDULE WHERE is_week_end = 'Y';

-- TR007: All days
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 7, 'Y' FROM CRS_DAY_SCHEDULE;

-- TR008: Weekdays only
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 8, 'Y' FROM CRS_DAY_SCHEDULE WHERE is_week_end = 'N';

-- TR009: Weekends only
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 9, 'Y' FROM CRS_DAY_SCHEDULE WHERE is_week_end = 'Y';

-- TR010: All days
INSERT INTO CRS_TRAIN_SCHEDULE (sch_id, train_id, is_in_service)
SELECT sch_id, 10, 'Y' FROM CRS_DAY_SCHEDULE;

COMMIT;

-- Populate CRS_PASSENGER
INSERT INTO CRS_PASSENGER (first_name, middle_name, last_name, date_of_birth, address_line1, address_city, address_state, address_zip, email, phone)
...
-- Confirmed bookings
INSERT INTO CRS_RESERVATION (passenger_id, train_id, travel_date, booking_date, seat_class, seat_status, waitlist_position)
